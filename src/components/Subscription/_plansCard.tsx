...
const _plansCard = ({item, data, navigation}:any): React.JSX.Element => {
  ...
  return (
    <VStack style={styles.mainContainer} >
      ...
      <HStack justifyContent={'center'} h={'auto'} mb={8} >
        <Text bold fontSize={'lg'} color={'white'} >$</Text>
        <Text bold style={styles.text} color={'white'} >{item?.amount}</Text>
      ...
      <AndroidButton style={{...styles.button}} pakID={item?.id} data={data} navigation={navigation} />
    </VStack>
  );
};
